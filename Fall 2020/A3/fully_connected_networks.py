...
class Linear(object):
    ...
    """
        ...
        dx = dout @ w.t()  # (N, M) * (M, D) = (N, D) and x: (N, D)
        dx = dx.reshape(size)  # (N, d1, ..., d_k)

        dw = x.reshape(n, -1).t() @ dout
        db = torch.sum(dout, dim=0).squeeze()
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return dx, dw, db
...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        hidden_state, hidden_cache = linear_relu.forward(X, self.params['W1'], self.params['b1'])
        scores, cache = linear.forward(hidden_state, self.params['W2'], self.params['b2'])
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
        ...
        dout, grads['W2'], grads['b2'] = linear.backward(dout=dout, cache=cache)
        _, grads['W1'], grads['b1'] = linear_relu.backward(dout=dout, cache=hidden_cache)
...
class FullyConnectedNet(object):
  ...
  ReLU non-linearities, and a softmax loss function.
  ...
  """
    ...
    """
        ...
        for layer_num in range(1, self.num_layers + 1):
            self.params['W' + str(layer_num)] = weight_scale * torch.randn(dims[layer_num - 1], dims[layer_num],
                                                                           dtype=self.dtype, device=device)

            self.params['b' + str(layer_num)] = torch.zeros(dims[layer_num], dtype=dtype, device=device)
    ...
    """
        ...
        for layer_num in range(1, self.num_layers):
            ...
            hidden_state, hidden_cache = linear_relu.forward(hidden_states[layer_num - 1],
                                                             self.params['W' + str(layer_num)],
                                                             self.params['b' + str(layer_num)])

            hidden_states.append(hidden_state)
            caches.append(hidden_cache)

        linear = Linear()
        scores, cache = linear.forward(hidden_states[-1], self.params['W' + str(self.num_layers)],
        ...
        dout, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = linear.backward(dout=dout,
                                                                                                     cache=cache)

        for layer_num in range(self.num_layers - 1, 0, -1):
            linear_relu = Linear_ReLU()
            dout, grads['W' + str(layer_num)], grads['b' + str(layer_num)] = \
                linear_relu.backward(dout=dout, cache=caches[layer_num - 1])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, device=device, update_rule=adam, lr_decay=0.95, num_epochs=30, batch_size=16)
...
def rmsprop(w, dw, config=None):
  ...
  """
    ...
    config['cache'] = v
    next_w = w - config['learning_rate'] * dw / (torch.sqrt(v) + config['epsilon'])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
  ...
  """
    ...
    config['t'] += 1
    unbiased_m = config['m'] / (1 - config['beta1'] ** config['t'])
    unbiased_v = config['v'] / (1 - config['beta2'] ** config['t'])

    next_w = w - config['learning_rate'] * unbiased_m / (torch.sqrt(unbiased_v) + config['epsilon'])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
