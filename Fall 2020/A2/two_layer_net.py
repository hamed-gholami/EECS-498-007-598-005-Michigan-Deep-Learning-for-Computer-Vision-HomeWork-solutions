...
def nn_forward_pass(params, X):
    ...
    As a practice, we will NOT allow using torch.relu and torch.nn ops
    ...
    b1 = b1.view(1, -1)
    z1 = X.matmul(W1) + b1  # z1: tensor of shape (N, D) * (D, H) = (N, H)

    hidden = torch.max(z1, torch.tensor([0], device='cuda'))  # hidden: Tensor of shape (N, H)

    b2 = b2.view(1, -1)
    scores = hidden.matmul(W2) + b2  # scores: Tensor of shape (N, C)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    largest, _ = torch.max(scores, dim=1, keepdims=True)
    scores -= largest

    correct_class_scores = scores[torch.arange(N), y]  # (N, 1)
    ...
    loss = 1 / N * torch.sum(-correct_class_scores + torch.log(sum_exp_scores)) + reg * (torch.sum(W1 * W1) +
                                                                                         torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    true_class[torch.arange(N), y] = 1

    grads['W2'] = torch.zeros_like(W2)
    grads['W2'] = 1 / N * h1.t().matmul(probabilities - true_class) + 2 * reg * W2  # (H, N) * (N, C) = (H, C)

    grads['b2'] = torch.zeros_like(b2)
    grads['b2'] = torch.sum(probabilities - true_class, dim=0)
    grads['b2'] = grads['b2'] / N
    grads['b2'] = grads['b2'].squeeze()

    da1 = (probabilities - true_class).matmul(W2.t())  # (N, C) * (C, H) = (N, H)
    dz1 = da1.clone()
    dz1[h1 == 0] = 0  # (N, H)

    grads['W1'] = torch.zeros_like(W1)
    grads['W1'] = 1 / N * X.t().matmul(dz1) + 2 * reg * W1
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
  ...
  - y: A PyTorch tensor f shape (N,) giving training labels; y[i] = c means that
    X[i] has the label c, where 0 <= c < C.
  ...
  """
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # Replace "pass" statement with your code
        params['W1'] -= (learning_rate_decay ** it) * learning_rate * grads['W1']
        params['b1'] -= (learning_rate_decay ** it) * learning_rate * grads['b1']
        params['W2'] -= (learning_rate_decay ** it) * learning_rate * grads['W2']
        params['b2'] -= (learning_rate_decay ** it) * learning_rate * grads['b2']
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_get_search_params():
  ...
  """
    ...
    ###########################################################################
    # TODO: Add your own hyperparameter lists. This should be similar to the #
    ...
    regularization_strengths = [0, 1e-5, 1e-6, 1e-7]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  """
    ...
    classes = torch.unique(data_dict["y_train"])
    output_size = 10
            ...
            for regularization_strength in regularization_strengths:
                stat = \
                    model.train(data_dict["X_train"], data_dict["y_train"], data_dict["X_val"], data_dict["y_val"],
                                num_iters=300, batch_size=128,
                                learning_rate=learning_rate, learning_rate_decay=0.95,
                                reg=regularization_strength)

                val_acc_history = stat['val_acc_history']
