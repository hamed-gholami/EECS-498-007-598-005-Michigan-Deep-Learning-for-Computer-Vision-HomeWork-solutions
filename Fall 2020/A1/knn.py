  ...
  """
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = (x_train[i].flatten() - x_test[j].flatten()).pow(2).sum()
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  """
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
        dists[i] = (x_train[i].flatten() - x_test.reshape((num_test, -1))).pow(2).sum(axis=1)
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  """
    ...
    x = x_test.reshape((num_test, -1))
    M = x_tr.matmul(x.t())  # M.shape = (num_train, num_test)

    te = x.pow(2).sum(axis=1, keepdims=True)
    # te = te.reshape((te.shape[0], 1))

    tr = x_tr.pow(2).sum(axis=1, keepdims=True)
...
def predict_labels(dists, y_train, k=1):
  ...
  """
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    occurrences = dists.topk(k=k, dim=0, largest=False)[1]

    for i in range(num_test):
      labels = y_train[occurrences[:, i]]
  ...
  """
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_folds_copy = x_train_folds.copy()
            y_folds_copy = y_train_folds.copy()
            x_test = x_folds_copy.pop(i)
            y_test = y_folds_copy.pop(i)
            clf = KnnClassifier(torch.cat(x_folds_copy), torch.cat(y_folds_copy))
            accuracies.append(clf.check_accuracy(x_test, y_test, k, quiet=True))
