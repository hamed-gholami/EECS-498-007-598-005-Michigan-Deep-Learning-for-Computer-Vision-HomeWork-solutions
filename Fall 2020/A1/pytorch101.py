...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    size = x.size() # type(size): <class 'torch.Size'>
    size = torch.tensor(size) # to use vectorized multiplication
    num_elements = torch.prod(size)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    last_row = x[-1]
    third_col = x[:, 2:3] # 2:3 not just 2 because I want to get it as column
                          # vector not row vector
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    indices_range = range(len(x))
    indices = list(indices_range)
    y = x[indices[::-1]]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    left = x[:12].contiguous()
    left = left.reshape(3, 4)

    right = x[12:].contiguous()
    right = right.reshape(3, 4)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.contiguous()
    min_indices = x.argmin(dim=1)
    M = x.size()[0]
    y[range(M), min_indices] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
